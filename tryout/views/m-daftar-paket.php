                        ...
                        <div class="col-100">
                            ...
                            <?php else: ?>
                                <?php foreach ($paket_dikerjakan as $paketitem):?>
                                    <blockquote>
                                        Nama Paket : <?=$paketitem['nm_paket'] ?><br>
                                        <a onclick="detail_paket(<?=$paketitem['id_paket']?>)" data-todo='<?=json_encode($paketitem)?>' title="Lihat Score" class="button button-small js-form-submit button-fill button-primary modal-on<?=$paketitem['id_paket']?>" style="width: 70px;">Score</a>
                                        <?php if ($status_to=="done"): ?>
                                            <a onclick="pembahasanto(<?=$paketitem['id_paket']?>)" data-todo='<?=json_encode($paketitem)?>' title="Pembahasan" class="button button-small js-form-submit button-fill button-primary" style="width: 70px;">Pembahasan</a>
                                        <?php endif; ?>
                                    </blockquote>
                                    <hr>
                                <?php endforeach ?>
